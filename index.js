...
async function run() {
    try {
        ...
        const rifles_db = client.db("AssaultRifles");
        const usersCollection = rifles_db.collection("users");
        const ordersCollection = rifles_db.collection("orders");
...
app.get('/', (req, res) => {
    res.send("Welcome to AssaultRifles's backend api...");
});
